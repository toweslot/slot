...
final class AmploPayProvider implements PaymentProviderInterface
{
    ...
    private function request(string $method, string $path, ?array $body = null): array
    {
        ...
        for ($i = 0; $i < $attempts; $i++) {
            ...
            curl_setopt_array($ch, [
                ...
                // HTTP/2 + IPv4: hospedagens compartilhadas costumam ser bloqueadas
                // pelo Cloudflare da AmploPay quando saem por IPv6 ou HTTP/1.1 antigo.
                CURLOPT_HTTP_VERSION => defined('CURL_HTTP_VERSION_2TLS') ? CURL_HTTP_VERSION_2TLS : CURL_HTTP_VERSION_1_1,
                CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4,
                CURLOPT_HEADER => true,
                // User-Agent honesto. Fingir Chrome sem impressao digital de browser
                // e justamente o que dispara o desafio 403 do Cloudflare.
            ...
            if ($isHtml || ($resp === false)) {
                $lastHtml = is_string($resp) ? substr($resp, 0, 500) : '';
                if (preg_match('/cf-ray:\s*([^\r\n]+)/i', $rawHeaders, $m)) {
                    $lastRay = trim($m[1]);
                }
                $this->logBlock($url, $code, $err, $lastRay, $lastHtml);
            }
        ...
        if ($this->isHtmlBody((string)$resp)) {
            throw new RuntimeException(
                'AmploPay bloqueou a requisicao do servidor (HTTP ' . $code . ')'
                . ($lastRay !== '' ? ' [cf-ray: ' . $lastRay . ']' : '')
                . '. O IP da hospedagem precisa ser liberado na AmploPay. Detalhes em api/storage/amplopay.log.'
            );
        }
    ...
    private function logBlock(string $url, int $code, string $err, string $ray, string $htmlSnippet): void
    {
        try {
            ...
            $line = sprintf(
                "[%s] url=%s http=%d curl_err=%s cf_ray=%s body=%s\n",
                date('c'),
                $url,
                $code,
                $err !== '' ? $err : '-',
                $ray !== '' ? $ray : '-',
                preg_replace('/\s+/', ' ', $htmlSnippet)
            );
            @file_put_contents($dir . '/amplopay.log', $line, FILE_APPEND);
        } catch (\Throwable) {
            // logging nunca pode quebrar o pagamento
        }
    }
