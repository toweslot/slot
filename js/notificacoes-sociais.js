...
(function () {
  ...
  function renderToast(n) {
    ...
    const isCad = n.tipo === 'cadastro';
    const isSaq = n.tipo === 'saque';
    const tag = isCad ? 'Novo cadastro' : (isSaq ? 'Saque confirmado' : 'Depósito confirmado');
    const acao = isCad ? 'entrou na plataforma' : (isSaq ? 'sacou' : 'depositou');

    const el = document.createElement('div');
    el.className = 'ns-toast';
    el.innerHTML = `
      <div class="ns-icon">${isCad ? ICON_USER : ICON_CHECK}</div>
      <div class="ns-body">
        <div class="ns-tag">${tag}</div>
        <div class="ns-name"><b>${esc(n.nome)}</b> <span class="muted">${acao}</span></div>
        ${n.valor ? `<div class="ns-val">${esc(n.valor)}</div>` : ''}
      </div>
    `;
  ...
  // ====== FILA DE EVENTOS REAIS (cadastros/depósitos vindos da API) ======
  const filaReais = [];
  let ultimoEventoId = 0;
  let primeiraCarga = true;

  function mostrarToast() {
    const real = filaReais.shift();
    renderToast(real || gerarNotificacao());
  }

  async function buscarEventosReais() {
    if (document.hidden) return;
    try {
      const res = await fetch('/api/public/eventos' + (ultimoEventoId ? `?after=${ultimoEventoId}` : ''), {
        headers: { Accept: 'application/json' }
      });
      if (!res.ok) return;
      const data = await res.json().catch(() => null);
      const lista = (data && (data.eventos || (data.data && data.data.eventos))) || [];
      lista.forEach((ev) => {
        if (ev.id > ultimoEventoId) ultimoEventoId = ev.id;
        if (primeiraCarga) return; // não replica histórico ao abrir a página
        filaReais.push({
          tipo: ev.tipo,
          nome: ev.nome,
          valor: ev.valor > 0 ? fmt(Number(ev.valor)) : ''
        });
      });
      primeiraCarga = false;
    } catch (_) { /* offline: segue só com as simuladas */ }
  }

  buscarEventosReais();
  setInterval(buscarEventosReais, 20000);

  function agendarToast() {
    const delay = filaReais.length ? 1500 : 6000 + Math.random() * 6000;
